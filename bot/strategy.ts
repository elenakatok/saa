...
/** One row of the getBidderView license table (mirrors api.ts BidderLicenseRow). */
export interface BidderLicenseRow {
  licenseId: LicenseId
  /** the caller's OWN private value for this license */
  yourValue: number
  currentHighBid: number
  currentWinnerIndex: number | null
  youAreWinner: boolean
  minIncrement: number
  /** the caller's minimum legal bid on this license; null = not allowed to bid it */
  minLegalBidForYou: number | null
}

/** The getBidderView shape the bot reasons over (mirrors api.ts BidderView). */
