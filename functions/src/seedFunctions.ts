...
export const seedMatchTest = onRequest(async (req, res) => {
  ...
  // SAA: single role `bidder`, composition { bidder:7 } (fixed)
...
// SAA group composition: single role `bidder`, fixed 7 per group.
export const seedGroupForTest = onRequest(async (req, res) => {
  ...
  bidderPids.forEach((pid, i) => {
    batch.set(instanceRef.collection('participants').doc(pid), {
      ...
      group_id: groupId,
      is_lead: pid === leadId,
      // Fully past setup so routeToPhase lands on the 'matched' auction room and the
      // auction overlay can take over — the UI harness drives the auction, not setup.
      prep_status: 'complete',
